...
#define SENSOR_CHECK_GOTO(a, str, lable)                                       \
    if (!(a))                                                                  \
    {                                                                          \
        ESP_LOGE(TAG, "%s:%d (%s):%s", __FILE__, __LINE__, __FUNCTION__, str); \
        goto lable;                                                            \
...
typedef struct _iot_sensor_slist_t
{
    ...
    SLIST_ENTRY(_iot_sensor_slist_t)
    next;
} _iot_sensor_slist_t;
    ...
    for (int i = 0; i < length; i++)
    {
        for (int j = 0; s_sensor_info[i].addrs[j] != '\0'; j++)
        {
            if (s_sensor_info[i].addrs[j] == i2c_address)
            {
                ...
                ESP_LOGI(TAG, "address 0x%0x might be %s (%s)", i2c_address, s_sensor_info[i].name, s_sensor_info[i].desc);
                break;
            }
        }
    }

    ESP_LOGI(TAG, "address 0x%0x found %d matched info", i2c_address, counter);
...
esp_err_t iot_sensor_stop(sensor_handle_t sensor_handle)
{
    ...
    { /*not supported case will be skip*/
